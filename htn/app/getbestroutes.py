...
class RouteFinder:
    ...
    def dfs_cycle(start, v, path, dist):
        ...
        for u, wt in graph[v]:
            if u == start:
                if len(path) < RouteFinder.MIN_INTERSECTIONS or len(path) > RouteFinder.MAX_INTERSECTIONS:
                    continue
                if dist + wt < RouteFinder.MIN_DIST or dist + wt > RouteFinder.MAX_DIST:
                    continue
                cycles.append([dist + wt, len(path)+1, path.copy() + [u]])
            elif vis[u] > 0:
                continue
            elif vis[u] == 0:
                RouteFinder.dfs_cycle(start, u, path.copy(), dist + wt)
    ...
    def get_best_routes(g):
        ...
        for node in choices(nodes, k=500): # nodes:
            root = node
            vis = RouteFinder.create_dict(graph)
            RouteFinder.dfs_cycle(root, root, [], 0)

        cycles = sorted(RouteFinder.cycles, key=RouteFinder.close_to_desired)[:10]
